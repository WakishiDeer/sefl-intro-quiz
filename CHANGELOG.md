...
- **リアクション機能**: ルーム内でリアルタイムに絵文字・テキストリアクションを送受信できる機能を追加
  - 絵文字リアクション8種（👏 ❤️ 😂 🎉 👍 🔥 😮 💯）
  - テキストリアクション5種（すごい! / なるほど〜 / ナイス! / ウケる / わかる〜）
  - テーマ固有リアクション: 各テーマに専用リアクションを用意（例: sakura=🌸🌷, party=🎊🎈）
  - cyber テーマ: ターミナル風等幅フォントのテキストリアクション4種（`[ACK]` / `> NICE_` / `// GG` / `$ sudo wow`）。黒背景+緑文字のハッカー美学で描画
  - `ReactionDefinition` に `mono?: boolean` フラグを追加し、テーマ横断で等幅フォント表示を抽象化
  - 送信したリアクションはルーム内全クライアントにフローティングパーティクルとして表示
  - 画面下部に常時表示のクイックアクセスバー（先頭4絵文字）と展開パネル
  - クライアント・サーバ両方でスライディングウィンドウ レートリミット（3秒間に最大5回）
  - 新規 Socket.IO イベント: `reaction:send` (C2S), `reaction:received` (S2C)
  - 新規ストア: `useReactionStore`（パーティクルのエフェメラル状態管理）
  - テスト: バリデーションスキーマ（5テスト）、リアクション定数（7テスト）、ストア（6テスト）を追加
